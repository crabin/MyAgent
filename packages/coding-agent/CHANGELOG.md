...
- Added a built-in authorization-gated security subagent mode for defensive security testing, security research prompts, safe web research, multi-source vulnerability lookup, bounded active checks, network discovery, dedicated asset discovery/attack surface/PoC research subagents, vulnerability assessment, supplied-event detection analysis, guarded persistent terminal sessions, in-session security memory, context usage reporting, and report assembly.
