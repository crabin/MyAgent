...
function createSecurityContextMessage(timestamp: number) {
	return {
		role: "custom" as const,
		customType: "security_subagent",
		content: "Security subagent context",
		display: true,
		details: {
			contextUsage: {
				selectedCount: 2,
				usedTokensEstimate: 360,
				budgetTokens: 1200,
			},
		},
		timestamp,
...
describe("AgentSession.getSessionStats", () => {
	...
	it("includes extension-provided security context budget usage in context stats", () => {
		const { session, sessionManager } = createSession();

		try {
			sessionManager.appendMessage(createUserMessage("scan example.com", 1));
			sessionManager.appendMessage(createSecurityContextMessage(2));
			sessionManager.appendMessage(createAssistantMessage("ready", 200, 3));
			syncAgentMessages(session, sessionManager);

			const stats = session.getSessionStats();
			expect(stats.contextUsage?.extension).toEqual({
